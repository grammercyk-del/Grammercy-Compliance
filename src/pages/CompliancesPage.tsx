...
import { softDeleteCompliance } from '@/api/compliances'
...
export function CompliancesPage() {
  ...
  const handleExport = async () => {
    try {
      await exportCompliances(filtered, `compliances_${new Date().toISOString().slice(0, 10)}`)
      push(`Exported ${filtered.length} compliance${filtered.length === 1 ? '' : 'ies'}`, 'success')
    } catch (err) {
      ...
      <div className="max-w-full mx-auto space-y-4">
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <button className="btn-secondary gap-2 text-sm" onClick={handleExport} disabled={filtered.length === 0}>
        ...
        <ComplianceFormModal
          ...
          onSuccess={(msg) => { push(msg, 'success'); setShowForm(false); setEditRow(null); refetch() }}
        ...
        <AuditModal open={!!auditRow} onClose={() => setAuditRow(null)} compliance={auditRow} />
